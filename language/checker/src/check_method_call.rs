...
pub async fn check_method_call(code_verifier: &mut CodeVerifier<'_>, variables: &mut SimpleVariableManager, effect: Effects)
    ...
    let method = if let Some(found) = calling {
        ...
        if is_modifier(return_type.inner_struct().data.modifiers, Modifier::Trait) {
            ...
            let method = Syntax::get_function(code_verifier.syntax.clone(), placeholder_error(
        ...
        if let Ok(value) = Syntax::get_function(code_verifier.syntax.clone(), placeholder_error(String::new()),
            ...
            let returning = match returning {
                Some(inner) => Some(Syntax::parse_type(code_verifier.syntax.clone(), placeholder_error(format!("Bounds error!")),
                                                       code_verifier.resolver.boxed_clone(), inner, vec!())
        ...
        Syntax::get_function(code_verifier.syntax.clone(), placeholder_error(format!("Unknown method {}", method)),
                             method, code_verifier.resolver.boxed_clone(), true).await?
    };

    let returning = match returning {
        Some(inner) => Some(Syntax::parse_type(code_verifier.syntax.clone(), placeholder_error(format!("Bounds error!")),
                                               code_verifier.resolver.boxed_clone(), inner, vec!()).await?
            .finalize(code_verifier.syntax.clone()).await),
        None => None
    };
