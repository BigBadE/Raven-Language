...
pub fn compile_internal<'ctx>(
    ...
    if name.starts_with("numbers::Cast") {
        ...
        type_getter
            .compiler
            .builder
            .build_return(Some(value.get_params().get(0).unwrap()));
        ...
        let length = type_getter
            ...
            .build_call(
                ...
                &[BasicMetadataValueEnum::PointerValue(
                    value.get_params().get(0).unwrap().into_pointer_value(),
                )],
                "0",
            )
            .try_as_basic_value()
        ...
        let returning = compiler.builder.build_int_sub(
            compiler
                .builder
                .build_load(params.get(0).unwrap().into_pointer_value(), "2")
                .into_int_value(),
            compiler
                .builder
                .build_load(params.get(1).unwrap().into_pointer_value(), "3")
        ...
        unsafe {
            gep = compiler.builder.build_in_bounds_gep(
                params.get(0).unwrap().into_pointer_value(),
                &[offset],
                "1",
            );
        }
        ...
        let returning = compiler.builder.build_not(
            compiler
                .builder
                .build_load(params.get(0).unwrap().into_pointer_value(), "1")
                .into_int_value(),
            "0",
        );
        compiler.builder.build_store(malloc, returning);
